...
public class ChessMatch {
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		board.pleacePeace(newPiece, pos);
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
			...
			board.pleacePeace(rook, targetT);
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		if (capturedPiece != null) {
			board.pleacePeace(capturedPiece, target);
		...
		if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
			...
			board.pleacePeace(rook, sourceT);
		...
		if (p instanceof Pawn) {
			if (source.getRow() != target.getColumn() && capturedPiece == enPassantVulnerable) {
				...
				board.pleacePeace(pawn, pawnPosition);
			}
		}
	}
	...
	private void placeNewPiece(char column, int row, ChessPiece piece) {
		board.pleacePeace(piece, new ChessPosition(column, row).toPosition());
